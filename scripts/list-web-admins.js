...
async function listWebAdmins() {
    console.log('--- Project 2 Web Admins (web_admins collection) ---');
    const snapshot = await db.collection('web_admins').get();
    if (snapshot.empty) {
        console.log('No web admins found in Firestore.');
    } else {
...
listWebAdmins().catch(err => {
    console.error('Error listing web admins:', err);
    process.exit(1);
});
